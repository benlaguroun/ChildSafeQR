...
export function SafeZoneMap({ center, radius, interactive = false, onLocationSelect }: SafeZoneMapProps) {
    ...
    const initMap = async () => {
      ...
      try {
        ...
        if (!mapInstanceRef.current) {
          mapInstanceRef.current = L.map(mapRef.current).setView([center.lat, center.lng], 15)
          ...
          if (interactive && onLocationSelect) {
            mapInstanceRef.current.on("click", async (e: any) => {
              ...
              try {
                ...
                onLocationSelect({ lat, lng }, locationString || "Unknown location")
              } catch (error) {
